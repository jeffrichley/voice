"""madrigal.generate — the orchestrator + madrigal.speak convenience wrapper.

Per plan v2 §1: single entry point with uniform Result return.

This module wires together:
- madrigal.Spec (request)
- madrigal.engine.TTSBackend (synthesis)
- madrigal.registry.Registry (voice_id → VoiceInfo)
- madrigal.cache.Cache (content-addressed)
- madrigal.chunking (text splitting)
- madrigal.Result (response, attribute-population-by-config)
...
from madrigal._cache_key import cache_key as _derive_cache_key
from madrigal._wav import concat_wavs, wav_duration_ms, wav_sample_rate_hz
from madrigal.cache import Cache, CacheEntry
from madrigal.chunking import chunk as _chunk_text
from madrigal.engine.protocol import TTSBackend
