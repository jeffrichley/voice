...
- Tagged `@pytest.mark.real_engine`; skipped without MADRIGAL_REAL_ENGINE_OK=1.
- Done-gate convention: declaring v0.1 done requires MADRIGAL_REAL_ENGINE_OK=1
...
Required env vars when MADRIGAL_REAL_ENGINE_OK=1:
- MADRIGAL_TEST_MODEL_PATH: filesystem path to Qwen3-TTS model
- MADRIGAL_TEST_REF_WAV: filesystem path to a reference WAV
- MADRIGAL_TEST_REF_TEXT: reference text matching the WAV
"""
...
@pytest.mark.skipif(
    not os.environ.get("MADRIGAL_REAL_ENGINE_OK"),
    reason=(
        "Requires real Qwen3-TTS engine (torch + qwen-tts + GPU). "
        "Set MADRIGAL_REAL_ENGINE_OK=1 + MADRIGAL_TEST_MODEL_PATH/REF_WAV/REF_TEXT to run. "
...
def test_batch_item_independence_empirical() -> None:
    ...
    model_path = os.environ.get("MADRIGAL_TEST_MODEL_PATH")
    ref_wav_path = os.environ.get("MADRIGAL_TEST_REF_WAV")
    ref_text = os.environ.get("MADRIGAL_TEST_REF_TEXT")

    assert model_path, "MADRIGAL_TEST_MODEL_PATH must be set"
    assert ref_wav_path, "MADRIGAL_TEST_REF_WAV must be set"
    assert ref_text, "MADRIGAL_TEST_REF_TEXT must be set"
    ...
    if is_item_independent:
        ...
        # config). The §5 fallback in madrigal.generate is overly conservative —
